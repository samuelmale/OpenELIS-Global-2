 ...
 * 1. MANUFACTURER_FIXED control lots auto-seed/re-seed qc_statistics from
 *    manufacturer mean and SD (on create and update)
 * 2. The invariant: a lot cannot be ACTIVE without statistics.
 *    Enforced at creation (validator rejects incomplete config)
 *    and at activation (rejects when no statistics exist).
 */
public class QCControlLotServiceManufacturerFixedTest extends BaseWebContextSensitiveTest {
    ...
    public void createControlLot_manufacturerFixed_shouldSeedStatisticsRow() {
        ...
        assertEquals("Statistics control lot ID should match",
                created.getId(), stats.getControlLotId());
        assertEquals("Statistics mean should equal manufacturer mean",
                0, new BigDecimal("100.00000").compareTo(stats.getMean()));
        assertEquals("Statistics SD should equal manufacturer SD",
                0, new BigDecimal("5.00000").compareTo(stats.getStandardDeviation()));
        assertEquals("Calculation method should be MANUFACTURER_FIXED",
                "MANUFACTURER_FIXED", stats.getCalculationMethod());
        assertEquals("Num values should be 0 (manufacturer-provided, not calculated)",
                Integer.valueOf(0), stats.getNumValues());
    ...
    public void updateControlLot_manufacturerFixed_shouldSeedStatisticsWhenNoneExist() {
        ...
        assertEquals("Mean should be updated to 55.5",
                0, new BigDecimal("55.50000").compareTo(updatedStats.getMean()));
        assertEquals("SD should be updated to 3.2",
                0, new BigDecimal("3.20000").compareTo(updatedStats.getStandardDeviation()));
    ...
    public void updateControlLot_manufacturerFixedWithNoExistingStats_shouldSeedStatistics() {
        ...
        // Create as INITIAL_RUNS first (no stats seeded), then switch to MANUFACTURER_FIXED
        QCControlLot lot = new QCControlLot();
        lot.setId(UUID.randomUUID().toString());
        ...
        QCControlLot created = controlLotService.createControlLot(lot);
        assertNotNull(created);
        assertNull("INITIAL_RUNS lot should have no stats",
                statisticsService.getLatestStatistics(created.getId()));
    ...
    public void activateControlLot_noStatistics_shouldReject() {
        ...
        assertNull("Should have no statistics",
                statisticsService.getLatestStatistics(created.getId()));
    ...
    public void activateControlLot_withStatistics_shouldSucceed() {
        ...
        assertNotNull("Should have statistics",
                statisticsService.getLatestStatistics(created.getId()));
