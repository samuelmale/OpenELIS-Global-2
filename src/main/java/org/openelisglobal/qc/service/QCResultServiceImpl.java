...
public class QCResultServiceImpl extends BaseObjectServiceImpl<QCResult, String> implements QCResultService {
     ...
     *
     * Supports three lot statuses:
     * - ACTIVE: normal operation, z-score computed from existing statistics
     * - ESTABLISHMENT: bootstrapping phase for ROLLING/INITIAL_RUNS lots;
     *   results are saved with null z-score until enough data accumulates
     *   to compute statistics, at which point the lot transitions to ACTIVE
     * - Any other status (EXPIRED, ARCHIVED): rejected
     *
    ...
    public QCResult createQCResult(String analyzerId, String testId, String controlLotId, String controlLevel,
        ...
        // sys_user_id for automated QC results (analyzer pipeline)
        ...
        // During establishment, try to compute statistics now that we have a new result.
    ...
    private void tryBootstrapStatistics(QCControlLot controlLot, int sysUserId) {
        ...
        LogEvent.logInfo(this.getClass().getName(), "tryBootstrapStatistics",
                "Bootstrapped statistics for lot " + controlLot.getId()
                        + " (method=" + controlLot.getCalculationMethod() + "), transitioned to ACTIVE");
    }

    /**
     * Recalculate rolling statistics for an ACTIVE lot after a new result.
     * The window slides forward to include the most recent N results.
     */
     ...
     *
     * @return computed statistics, or null if insufficient data or no calculator found
     */
    private QCStatistics computeStatistics(QCControlLot controlLot) {
        String calculationMethod = controlLot.getCalculationMethod();

        StatisticsCalculator calculator = statisticsCalculators.stream()
                .filter(c -> c.supports(calculationMethod))
                .findFirst()
                .orElse(null);
        if (calculator == null) {
            return null;
